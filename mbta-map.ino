...
void loop() {
  ...
  byte anode_to_light = (millis() / 1000) % LEDS_PER_GROUP;
  for (byte group_i = 0; group_i < NUM_GROUPS; group_i++) {
    for (byte anode_i = 0; anode_i < LEDS_PER_GROUP; anode_i++) {
      if (anode_i == anode_to_light) {
        led_state[group_i * LEDS_PER_GROUP + anode_i] = LED_ON;
      } else {
        led_state[group_i * LEDS_PER_GROUP + anode_i] = LED_OFF;
      }
    }
  }
